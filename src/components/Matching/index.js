...
class Matching extends Component {
  ...
  changeImages = id => {
    ...
    if (id === randomImageId) {
      ...
      const number = Math.ceil(Math.random() * imagesList.length - 1)
      const uniqueId = imagesList[number].id
      const uniqueImg = imagesList[number].imageUrl
      this.setState({
        randomImageId: uniqueId,
        imageUrl: uniqueImg,
      })
    } else {
      this.clearInterval()
  ...
  render() {
    const {imageUrl, score, timer, condition} = this.state
    const result = this.getFiltered()
    return (
